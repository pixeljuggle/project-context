...
```go
module github.com/pixeljuggle/my-app
...
This is a markdown file that contains `code blocks` inside it.
Everything still works perfectly thanks to 4-backtick fences.
```
...
## Why this tool is perfect for AI agents

Modern LLMs have huge context windows, but **garbage in = garbage out**.

This tool gives you:

- Clean, focused context (no `.git`, no `node_modules`, no binaries)
- Proper Markdown that doesn't break when pasted
- Token estimation so you know if you're near the limit
- One-command workflow that fits perfectly into Cursor, Claude Projects, Aider, Continue.dev, etc.
